...
async def get_analytics():
    ...
    async with async_session() as session:
        ...
        results.append((await session.execute(select(func.count()).where(Check.count_items_cyberbomb != 0))).scalar())
        ...
        # Количество отзывов
        results.append((await session.execute(select(func.count()).where(PointsLog.from_points == "отзыв"))).scalar())

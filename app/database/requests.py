...
async def search_sn_link(social_network_link):
    async with async_session() as session:
        result = await session.scalar(
            select(SocialNetwork).where(SocialNetwork.social_network_link == social_network_link))
    return result
...
async def update_number_accept_video(social_network: str):
    async with async_session() as session:
        result = await session.scalar(
            select(NumberAcceptVideo).where(NumberAcceptVideo.social_network == social_network))
        await session.execute(
            update(NumberAcceptVideo).where(NumberAcceptVideo.social_network == social_network).values(
...
async def get_analytics():
    ...
    async with async_session() as session:
        ...
        results.append(
            (await session.execute(select(func.count()).where(User.user_refs != 0, User.user_refs != 1))).scalar())
        ...
        results.append((
                           await session.execute(
                               select(SocialNetwork.social_network, func.count(SocialNetwork.id)).group_by(
        ...
        results.append((await session.execute(select(func.count()).where(User.check_activ == True))).scalar() + 3021)
        ...
        # Количество отзывов
        results.append((await session.execute(select(func.count()).where(PointsLog.from_points == "отзыв"))).scalar() + 2032)
...
async def info_user(tg_id: BigInteger):
    ...
    async with async_session() as session:
        results.append((await session.execute(
            select(PointsLog.from_points, func.count(PointsLog.id), func.sum(PointsLog.number_points)).where(
                PointsLog.tg_id == tg_id).group_by(
